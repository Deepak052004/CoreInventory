...
export default function DeliveryForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const payload = { customer, lines: lines.filter((l) => l.product && l.quantity > 0 && l.warehouse) };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-4">
        <Link to="/deliveries"><Button variant="ghost" size="sm" className="gap-2"><ArrowLeft className="w-4 h-4" /> Back</Button></Link>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{isEdit ? 'Edit Delivery' : 'New Delivery'}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex gap-2">
          ...
          {isEdit && <Button type="button" variant="secondary" onClick={handleValidate} disabled={saving}>Validate delivery</Button>}
          <Link to="/deliveries"><Button type="button" variant="outline">Cancel</Button></Link>
        </div>
      </form>
    </div>
  );
